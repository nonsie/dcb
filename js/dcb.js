(function (d, w, s, p) {
  function getIframe() {
    ...
    f.className = 'dcb-component-edit-iframe';
    d.body.appendChild(f);
    return f;
  }

  function findEls(selector, fromEl) {
    fromEl = typeof fromEl === 'undefined' ? d : fromEl;
    return Array.prototype.slice.call(
      fromEl.querySelectorAll(selector)
    );
  }

  function findRegions(fromEl) {
    fromEl = typeof fromEl === 'undefined' ? d : fromEl;
    return findEls('.dcb-region', fromEl);
  }

  function findComponentsInRegion(regionEl) {
    return findEls('.dcb-component', regionEl);
  }

  function getPathComponentEdit(componentId) {
    return [p, 'admin/structure/dcb_component/', componentId, '/edit'].join('');
  }

  function createActionLinks(componentId) {
    var a = d.createElement('div');
    a.className = 'dcb-action-links';
    var e = d.createElement('a');
    e.href = getPathComponentEdit(componentId);
    e.textContent = 'Edit';
    e.addEventListener('click', handleComponentEditClick);
    a.appendChild(e);
    return a;
  }

  function iterateCallback(arr, fn) {
    for (var i = 0; i < arr.length; i++) {
      fn(arr[i]);
    }
  }

  function handleComponentEditClick(e) {
    e.preventDefault();
    var i = getIframe();
    i.src = e.target.getAttribute('href');
  }

  var iframe;
  var regions = findRegions();
  iterateCallback(regions, function (region) {

    var components = findComponentsInRegion(region);
    iterateCallback(components, function (component) {

      component.appendChild(createActionLinks(component.getAttribute('data-dcb-bid')));

    });

  });

})(document, window, drupalSettings.dcb, drupalSettings.path.baseUrl);
...
//       var $this = this;
//       this.regions.push(new DCBRegion(region));
...
//                   this.regions[rrid].blocks = this.regions[rrid].blocks.filter(function () {
//                     return true;
//                   });
//                 }
//                 else {
...
//       this.blocks.push(new DCBComponent(block, bid, rid, handler));
//     };
//
...
//     toggleUi: function () {
//       var $this = this;
//       var open = !!this.ui.hasClass('open');
//       $('html').animate({
...
//       for (var rid in DCB.regions) {
//         this.addRegion(DCB.regions[rid]);
//       }
//     },
//
...
//       var region = DCB.getRegion(rid);
//       if (region) {
//         region.region.toggleClass('active');
//         var open = !!region.region.hasClass('active');
...
//       var block = DCB.getBlock(rid, bid);
//       if (block) {
...
//       var $this = this;
//       return $this.makeAJAXLink(block.bid, '<i class="fa fa-edit" aria-hidden="true" title="Edit">', 'editform', rid);
//     },
//
...
//               var block = DCB.getBlock(rid, $(this).data('dyno-ui-item'));
//               if (block) {
//                 block.weight = i;
//               }
//               i++;
...
//       DCB.removeBlock(this.rid, this.bid, function (result) {
//         if (result.removed) {
//           $this.element.remove();
//         }
//       });
...
//
//   var init = false;
//   Drupal.behaviors.DCB = {
//     attach: function (context, settings) {
//       if (!init) {
//         init = true;
//         // Run Dynoblocks.
//         DCB.init(true, settings);
//         Drupal.DCB = DCB;
//         Drupal.DCBUi = DCBUi;
//       }
//     }
